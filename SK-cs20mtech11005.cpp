...
vector<int> time_manager(unsigned long long int time_in_milli)
{
	...
	clk[3] = int(time_in_milli%1000);//milliseconds
	return clk;
}
...
void* thread_function2(void* arg)
{
	...
	int temp = pow(n,2);
        for(int i=0;i<temp;i++)
        {
	        ...
	        {
	        	cout << " connection establishment failed, try after some time" << endl;
	        	exit(1);
	        }
...
void* process_func(void*arg)
{
    ...
    //ofstream openfile1("output",ofstream::app);
	...
	for(int j =0;j<m;)
	{
		...
		while(k<numerator)
		{
			...
			clk = time_manager(time_in_milli);//vector of time

            		time_vector[process_id] = time_vector[process_id] + 1;
			//last_sent[process_id] = last_sent[process_id] + 1;
			last_update[process_id] = time_vector[process_id];
			...
			k++;
			Time1 = exp_dist(seed);
			this_thread::sleep_for(chrono::microseconds(int(10000*Time1)));
		}
		...
		while(k<denominator and j<m)
		{
			...
			int random_index = rand() % (vec[process_id].size()-1);
                ...
                /* str = "";
                str+= to_string(process_id);
                str+= " ";
                str+= to_string(time_vector[process_id]);
                ...
                q=q+1;
                str = vector_to_string(time_vector);
                time_in_milli = chrono::duration_cast<chrono::milliseconds>(chrono::system_clock::now().time_since_epoch()).count();
            ...
            //else if(last_sent[vec[process_id][random_index+1]-1] < last_update[vec[process_id][random_index+1]-1])
            //{

                string str = "";
                for(int i=0;i<n;i++)
                {
                    if(last_sent[vec[process_id][random_index+1]-1] < last_update[i])
                    {
                        ...
                        str+= " ";
                        Number_of_entries_sent += 1;
                    }
                }
                Number_of_bytes_sent += (str.size());
                last_sent[vec[process_id][random_index+1]-1] = time_vector[process_id];
                if((send_status = send(socket_arr1[(process_id*n) + vec[process_id][random_index+1]-1],str.c_str(),str.size(),0)) <str.size())
                {
                        cout << "sending failed" << endl;
                }
	...
	this_thread::sleep_for(chrono::microseconds(int(10000*Time1)));
		...
		timeout.tv_sec = 0;
		timeout.tv_usec = lambda*1000 + 500;
		...
		for(int l=0;l<n;l++)
		{
			...
			if(FD_ISSET(socket_set,&sockets))
			{
				...
				{
					if(time_vector[temp[k]] < temp[k+1])
					{
						time_vector[temp[k]] = temp[k+1];
						last_update[temp[k]] = time_vector[process_id];
					}
				}
				string s;
...
int main()
{
	...
	//---------array of structure for each process--//
	struct parameters param[n];
	vector<int> vector_clock(n,0);
	for(i=0;i<n;i++)
	{
	...
	cout << "No_of_entries_sent = " << Number_of_entries_sent << endl;
	fprintf(fp,"No_of_entries_sent  %d\n",Number_of_entries_sent);
	cout << "No_of_messages_sent = " << 50*n << endl;
	fprintf(fp,"No_of_messages_sent = %d\n",50*n);
	cout << "Average_No_of_messages_sent = " << Number_of_entries_sent/(50*n) << endl;
	fprintf(fp,"Average_No_of_messages_sent = %d\n",Number_of_entries_sent/(50*n));
	fclose(fp);
}
